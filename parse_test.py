...
for i in range(num_):
    ...
    df = pd.read_csv("./Data/MS_files_test/"+sub+"test_"+str(i+1)+".csv",header=None)
    ...
    pd.DataFrame(sym_sortedA).to_csv("./Data/CSV_files/"+sub+"test_L1_"+str(i+1)+".csv")
    print(i)
...
# generalized
for i in range(num_):
    df=pd.read_csv("./Data/CSV_files/"+sub+"test_L1_"+str(i+1)+".csv", index_col=0)
    df=np.asmatrix(df)
    s=np.shape(df)
    sortd=np.zeros(s)
    K=[]
    ...
    pd.DataFrame(d).to_csv("./Data/CSV_files/"+sub+"test_align_"+str(i+1)+".csv")
#     print(i)
